...
class TestExifToolGUI(unittest.TestCase):
    ...
    def setUp(self, mock_run):
        ...
        self.app = ExifToolGUI(
            master=MagicMock(),
            string_var_class=self.mock_string_var,
        )
    ...
    def test_browse_file(self):
        ...
        with patch.object(
            self.app, 'run_exiftool',
            return_value="EXIF data",
        ):
            self.app.browse_file()
            self.assertEqual(
                self.app.current_file, "test.jpg",
            )
            self.app.run_exiftool.assert_called_once_with(
                ["test.jpg"],
            )
    ...
    def test_remove_all_exif(self):
        ...
        with patch.object(self.app, 'run_exiftool'):
            ...
            self.app.run_exiftool.assert_called_once_with(
                ['-all=', '-overwrite_original', 'test.jpg'],
            )
