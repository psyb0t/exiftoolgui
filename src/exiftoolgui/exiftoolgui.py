...
class ExifToolGUI:
    ...
    def check_exiftool():
        try:
            ...
            raise RuntimeError(
                "ExifTool is installed but not working properly."
            )
    ...
    def __init__(self, master=None, string_var_class=None):
        ...
        self.StringVar = (
            string_var_class if string_var_class else tk.StringVar
        )
    ...
    def create_widgets(self):
        ...
        self.notebook.add(
            self.remove_frame, text="Remove EXIF",
        )
        self.create_remove_tab()

        self.batch_frame = ttk.Frame(self.notebook)
        self.notebook.add(
            self.batch_frame, text="Batch Process",
        )
    ...
    def create_edit_tab(self):
        ...
        ttk.Button(
            self.edit_frame,
            text="Select Image",
            command=self.select_image_for_edit,
    ...
    def create_remove_tab(self):
        ttk.Button(
            self.remove_frame,
            text="Select Image",
            command=self.select_image_for_remove,
        ).grid(row=0, column=0, pady=10)

        ttk.Button(
            self.remove_frame,
            text="Remove All EXIF Data",
            command=self.remove_all_exif,
    ...
    def browse_file(self):
        ...
        if file_path:
            ...
            pretty_exif_data = self.pretty_print_json(
                exif_data,
            )
    ...
    def apply_edit(self):
        ...
        if not tag or not value:
            messagebox.showerror(
                "Error",
                "Both tag and value must be provided",
            )
            return
        result = self.run_exiftool(
            [f"-{tag}={value}", self.current_file],
        )
